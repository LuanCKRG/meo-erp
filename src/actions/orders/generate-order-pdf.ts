...
async function generateOrderPdf(orderId: string): Promise<ActionResponse<{ pdfBase64: string }>> {
	...
	try {
		...
		const { customer, created_at, equipment_value, labor_value, other_costs, system_power, current_consumption } = orderDetails.data
		...
		const serviceFee = serviceFeeRes.data / 100 // 0.021
